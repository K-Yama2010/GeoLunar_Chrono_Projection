...
void getLocalTime_for_cities(struct tm* timeinfo, const City_Glass* city) {
    int m = timeinfo->tm_mon + 1, d = timeinfo->tm_mday, w = timeinfo->tm_wday;
    if (strcmp(city->name, "Tokyo") == 0) { timeinfo->tm_hour += 9; }
    else if (strcmp(city->name, "London") == 0) {
        bool is_bst = (m > 3 && m < 10) || (m == 3 && d - w >= 25) || (m == 10 && d - w < 25);
        if (is_bst) timeinfo->tm_hour += 1;
    } else if (strcmp(city->name, "NewYork") == 0) {
        timeinfo->tm_hour -= 5;
        bool is_dst = (m > 3 && m < 11) || (m == 3 && d - w >= 8) || (m == 11 && d - w < 1);
        if (is_dst) timeinfo->tm_hour += 1;
    }
    mktime(timeinfo);
}
